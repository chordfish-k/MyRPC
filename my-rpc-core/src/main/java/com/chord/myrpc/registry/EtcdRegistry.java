...
public class EtcdRegistry implements Registry {
    ...
    public void register(ServiceMetaInfo serviceMetaInfo) throws Exception {
        // 创建Lease和KV客户端
        // 用于管理 etcd 的租约机制租约是 etcd 中的一种时间片，用于为键值对分配生存时间，并在租约到期时自动删除相关的键值对。
        // 通过 leaseClient 可以创建、获取、续约和撤销租约。
        ...
        // 设置要存储的键值对
        final String ETCD_ROOT_PATH = "/" + RpcApplication.getRpcConfig().getName() + "/";
        final String registerKey = ETCD_ROOT_PATH + serviceMetaInfo.getServiceNodeKey();
    ...
    public List<ServiceMetaInfo> serviceDiscovery(String serviceKey) {
        ...
        // 前缀搜索，结尾一定要加 '/'
        final String ETCD_ROOT_PATH = "/" + RpcApplication.getRpcConfig().getName() + "/";
        final String searchPrefix = ETCD_ROOT_PATH + serviceKey + "/";
                ...
                for (WatchEvent event : response.getEvents()) {
                    switch (event.getEventType()) {
                        ...
                        case DELETE:
                            ...
                            break;
                        case PUT:
                            registryServiceCache.notify(KeyEventType.Put);
                        default:
                            break;
                    }
                }
